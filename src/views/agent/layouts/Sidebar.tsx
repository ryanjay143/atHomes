...
function AgentSidebar() {
  ...
  const links = [
    { path: "/agent-broker/user-dashboard", icon: faTachometerAlt, label: "Dashboard" },
    { path: "/agent-broker/agent-salesEncoding", icon: faKeyboard, label: "Sales Encoding" },
    { path: "/agent-broker/agent-brokerage", icon: faHome, label: "Brokerage/Property" },
    { path: "/agent-broker/agent-salesReport", icon: faChartLine, label: "Sales Report" },
  ];
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-white/70 backdrop-blur-xl shadow-2xl text-primary z-40 transition-all duration-300 ease-in-out transform ${
          isOpen ? "translate-x-0" : "md:-translate-x-full"
        } overflow-hidden rounded-r-3xl flex flex-col`}
        style={{
          boxShadow: "0 8px 32px 0 rgba(31, 38, 135, 0.25)",
          borderRight: "1.5px solid rgba(23, 37, 84, 0.08)",
        }}
      >
        <div className="w-full h-full flex flex-col items-center justify-between py-4">
          ...
          <div
            ...
            onClick={() => handleNavigation("/agent-broker/user-dashboard")}
          >
            <img
              src={logo}
              alt="Logo"
          ...
          {/* Footer */}
          <div className="text-center text-xs text-blue-900 font-semibold opacity-80 mt-4">
